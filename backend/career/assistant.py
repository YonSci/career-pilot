...
- Privacy and AI processing: with the member's own OpenAI key, the CV text (once, for extraction), verified facts and postings (for evaluations and drafts), alert-email text (mailbox source) and careers-page text (page source) are sent to OpenAI with storage disabled. The help assistant sends the question, recent turns and a non-sensitive account status summary to OpenAI under the owner's key; conversations are not stored. Each account is a private workspace. CV text, verified facts, postings, evaluations and drafts are visible only to that account. API keys and mailbox passwords are stored encrypted. The service owner administers the server; in running the service the owner looks only at account-level activity (for example whether a CV was verified) and does not read CV contents or drafts except to resolve a support request. Users can remove connections any time and request export or deletion. Privacy notice at /privacy.html, terms at /terms.html.
