...
- Privacy: each account is a private workspace. CV text, verified facts, postings, evaluations and drafts are visible only to that account. API keys and mailbox passwords are stored encrypted. The service owner administers the server; in running the service the owner looks only at account-level activity (for example whether a CV was verified) and does not read CV contents or drafts except to resolve a support request. Users can remove connections any time and request export or deletion. Privacy notice at /privacy.html, terms at /terms.html.
